...
public class BoardManager : MonoBehaviour
{
   ...
   public void Init()
   {
      ...
      NavMeshAgent agent = GetComponent<NavMeshAgent>();
      Vector3 groundPosition = new Vector3(width/2,0, height/2);
      GameObject ground = Instantiate(Ground,groundPosition, Quaternion.identity);
   ...
   public void SpawnInitGrid(Vector2Int gridLength)
   {
      int centerX = width / 2;
      int centerY = height / 2;
      for (int x = -gridLength.x; x < gridLength.x+1; x++)
      {
         for (int y = -gridLength.y; y < gridLength.y+1; y++)
         {
            Vector2Int curCell = new Vector2Int(centerX+x, centerY+y);
            Vector3Int curPos = new Vector3Int(curCell.x, curCell.y, 0);
            _tilemap.SetTile(curPos, grassTile);
            _tileCount++;
            _emptyCellList.Add(curCell);
            m_BoardData[curPos.x, curPos.y].HasCreatedTile = true;
            m_BoardData[curPos.x, curPos.y].Passible = true;
            m_BoardData[curPos.x, curPos.y].Terrain = Terrain.Grass;
         }
      }
   }
   ...
   public void CreateActionTile(Vector2Int position,int walkRange)
   {
      actionTilemap.ClearAllTiles();
      for (int i = -walkRange; i < walkRange+1; i++)
      {
         for (int j = -walkRange; j < walkRange+1; j++)
         {
            Vector3Int setActionTilePosition = new Vector3Int(position.x + i, position.y + j,0);
            if (Mathf.Abs(i)+Mathf.Abs(j) <= walkRange && m_BoardData[setActionTilePosition.x, setActionTilePosition.y].Passible)
            {
               actionTilemap.SetTile(setActionTilePosition,actionTile);
               m_BoardData[setActionTilePosition.x, setActionTilePosition.y].reachable = true;
            }
            else
            {
               actionTilemap.SetTile(setActionTilePosition,null);
               m_BoardData[setActionTilePosition.x, setActionTilePosition.y].reachable = false;
            }
